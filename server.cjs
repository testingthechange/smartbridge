...
/**
 * Contract:
 *   GET /api/public/manifest/:shareId
 *   -> { ok: true, manifest: { version, shareId, projectId, publishedAt, bundle, lineage } }
 *
 * This server uses master-save artifacts in R2. We treat `shareId` as a stable public id
 * whose "latest bundle" can be resolved from:
 *   storage/public/manifests/<shareId>.json   (preferred)
 *
 * The file should contain at minimum:
 *   { shareId, projectId, publishedAt, source: { latestKey, snapshotKey, savedAt } }
 */
...
app.get("/api/public/manifest/:shareId", async (req, res) => {
  try {
    ...
    // Resolve bundle: prefer explicit snapshotKey, else latestKey.latestSnapshotKey, else latest.json
    ...
    const lineage = {
      ...
      snapshotKey,
      source: safeString(index.source || "public-manifest-index"),

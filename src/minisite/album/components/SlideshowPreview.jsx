// src/minisite/album/components/SlideshowPreview.jsx
import React, { useEffect, useRef, useState } from "react";
import { idbGetBlob } from "../albumIdb";
import { safeRevoke, fmtTime } from "../meta/albumMeta.utils";
import { ghostBtnSm, primaryBtnSmall } from "../albumStyles.jsx";

export default function SlideshowPreview({ items }) {
  const [idx, setIdx] = useState(0);
  const [auto, setAuto] = useState(false);

  const item =
    Array.isArray(items) && items.length
      ? items[Math.min(idx, items.length - 1)]
      : null;
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, fontSize: 12, opacity: 0.75 }}>
        {items?.length ? (
          <>
            Slide <strong>{idx + 1}</strong> / {items.length} —{" "}
